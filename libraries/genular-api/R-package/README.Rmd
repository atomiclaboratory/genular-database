...
all_gene_results <- fetch_all_gene_search_results(
  ...
  limit = 100,  # Limit the number of results returned (API key specific page_limit in php backend)
  options = list(api_key = API_KEY, timeout = 1000)  # API key and timeout for the request
)

all_gene_symbols <- sapply(all_gene_results, function(x) x$symbol)
...
```{r example-6, message=FALSE, warning=FALSE}
