...
window.onload = function () {
    ...
    if (!document.getElementById('string_canvas')) {
        return;
    }
    
    let c = init("string_canvas").c,
        canvas = init("string_canvas").canvas,
        w = (canvas.width = window.innerWidth),
        h = (canvas.height = window.innerHeight),
    ...
    class tentacle {
        ...
        show(target) {
            if (dist(this.x, this.y, target.x, target.y) <= this.l) {
                ...
                c.strokeStyle = "hsla(" + (this.rand * 60 + 180) + ",100%," + (this.rand * 60 + 25) + "%, 0.5)";
    ...
    window.addEventListener("resize", function () {
        ...
        // Recreate tentacle objects to adapt to the new canvas size.
        tent = [];
        for (let i = 0; i < numt; i++) {
            tent.push(new tentacle(Math.random() * w, Math.random() * h, Math.random() * (maxl - minl) + minl, n, Math.random() * 2 * Math.PI));
        }
        ...
        c.clearRect(0, 0, w, h);
        draw();  // Calling draw() here directly to reflect changes immediately on resize.
    });
